...
2. **(valid-output — row fields + sparkline)** Given a result row for a variation that has at least one matched `sale_observation`, When the row renders, Then it displays the latest sold price (`sale_observation.sale_price`), the sold date (`sale_observation.sale_date`), the condition/grade (`grade_id`, shown as "Raw" when null), the serial number (`serial_number`, shown only when present), a description snippet, and a sparkline trend.
3. **(valid-output — scannable badges)** Given a variation that carries a parallel and a print run, When the row renders, Then discrete chip-style badges display for the parallel/print run (for example `/25` or `1/1`), the grade tier, and the format — each rendered as a separate chip rather than inline prose.
4. **(input-validation — format toggle + filters span both columns)** Given the format toggle, When a value is selected, Then it accepts only one of "Both", "Digital-only", or "Physical-only" (any other value is rejected and the toggle keeps its prior value); AND given any active filter (for example a grade or price-range filter), When the filter changes, Then it applies across BOTH the Digital and Physical columns at the same time.
5. **(error-handling — endpoint error)** Given the EPIC-04 two-column-results endpoint returns an error status (HTTP status code 500 or higher), When the view loads, Then a labeled "results unavailable" error state renders with a retry control, and neither column shows a partial or stale list.
6. **(edge-case — no results)** Given a query that returns no variations for the character, When the view renders, Then a "no results" state displays in place of two empty columns.
7. **(edge-case — zero-sale variation, asking-price estimate, and confidence gate)** Given a variation with zero matched sales, When its row renders, Then an "asking-price estimate" label (sourced from `raw_listing.asking_price`) displays in place of a confident sold price; AND given a variation whose `valuation.sample_size` is below 5 (n<5), Then the price shows a greyed "based on N sales" label and no single-sale value is presented as a confident price.
8. **(edge-case — digital-only parallel)** Given a variation whose `parallel_type.format_availability = 'digital'` (for example a Gilded tier), When the row renders, Then a "no physical counterpart" label displays in the Physical column for that parallel.
...
- [ ] Render the per-row sparkline trend backed by the `valuation` series (`trend_dir` -1/0/+1) (@frontend-engineer)
- [ ] Wire the format toggle (Both / Digital-only / Physical-only) and the filters to apply across both columns simultaneously (@frontend-engineer)
...
- [ ] The format toggle accepts only Both / Digital-only / Physical-only and, together with the filters, applies across both columns simultaneously.
...
- [ ] **Testing:** UI tests pass for the two-column render, the row fields, the chip badges, the sparkline, the format-toggle/filter behavior across both columns, and the empty/zero-sale/digital-only/error states; the UI coverage target is **≥50%** where code applies.
