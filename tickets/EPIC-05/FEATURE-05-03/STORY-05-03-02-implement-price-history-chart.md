...
5. **(edge-case — confidence gate)** Given the selected window's `valuation.sample_size = 0`, When the chart renders, Then an `asking-price estimate` label renders instead of a confident sold-price value; AND given the selected window's `valuation.sample_size` is between 1 and 4 inclusive, Then a `low-confidence` label renders alongside the series and no single-sale value is presented as a confident price.
6. **(ui-state — window switch active/pending)** Given the 90-day/1-year toggle, When a window is selected and its data is being fetched or rendered, Then the selected option is marked active and the toggle is disabled against a duplicate switch until the request resolves or fails; When it resolves, Then the chart shows the selected window; When it fails, Then the toggle returns to the prior selection and a labeled error renders.
7. **(accessibility — toggle semantics and chart text alternative)** Given the chart renders, When assistive technology inspects it, Then the 90-day/1-year toggle is a labeled control group (for example `role="radiogroup"` with each option exposing its selected state through `aria-checked`) reachable by keyboard with a visible focus indicator at a contrast ratio of at least 3:1; the sparkline exposes a text alternative (`role="img"` with an `aria-label` stating the trend direction and percent change) AND an adjacent visually-hidden data table or text summary lists the selected window's `p25`/`median`/`p75` and `sample_size`; and the trend direction is conveyed in text, not by the ▲/▼ glyph alone.
8. **(responsive — 375 CSS pixel viewport)** Given a viewport width of 375 CSS pixels, When the chart renders, Then the 90-day/1-year toggle labels stay visible and not truncated, the sparkline and the trend and confidence labels fit within the 375-pixel width or scroll inside their own container, and the page shows no page-level horizontal scrollbar.

## Sub-tasks

- [ ] Render the sparkline from the `valuation` series (`p25`/`median`/`p75`) for the selected window (@frontend-engineer)
- [ ] Implement the 90d/1y toggle mapping to `valuation.window_days` 90 and 365, marking the active selection and disabling the toggle while the window's data is in flight (@frontend-engineer)
- [ ] Render the trend indicator from `trend_dir` (▲/▼/neutral) paired with `trend_pct`, conveying the direction in text and not by the glyph alone (@frontend-engineer)
- [ ] Render the confidence-gate labels (n = 0 asking-price estimate; 1–4 low-confidence; ≥5 confident) (@frontend-engineer)
- [ ] Expose the toggle as a labeled `role="radiogroup"` (each option `aria-checked`) and the sparkline as `role="img"` with an `aria-label`, plus an adjacent visually-hidden data table listing the window's `p25`/`median`/`p75` and `sample_size` (@frontend-engineer)
- [ ] Keep the toggle labels, sparkline, and trend/confidence labels within a 375 CSS pixel viewport (fit or scroll inside their own container) with no page-level horizontal scrollbar (@frontend-engineer)
...
- [ ] On a window switch the selected option is marked active and the toggle is disabled against a duplicate switch until the request resolves (chart shows the new window) or fails (toggle returns to the prior selection and a labeled error renders).
- [ ] The toggle is a labeled `role="radiogroup"` (each option `aria-checked`) keyboard-reachable with a visible focus indicator at a contrast ratio of at least 3:1; the sparkline exposes a `role="img"` `aria-label` of the trend, plus an adjacent visually-hidden data table of `p25`/`median`/`p75` and `sample_size`; the trend direction is conveyed in text, not by the ▲/▼ glyph alone.
- [ ] At a 375 CSS pixel viewport the toggle labels stay visible, the sparkline and trend/confidence labels fit or scroll inside their own container, and the page shows no page-level horizontal scrollbar.
...
- [ ] **Testing:** UI tests pass for the toggle re-render, the trend rendering (▲/▼/neutral), the confidence-gate states (n = 0 / 1–4 / ≥5), the insufficient-history state, the window-switch pending/active/disabled state, the accessibility semantics (radiogroup toggle, sparkline text alternative and data table), and the 375 CSS pixel responsive layout; the UI coverage target is **≥50%** where code applies.
