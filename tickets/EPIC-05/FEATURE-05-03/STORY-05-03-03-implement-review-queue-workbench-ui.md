...
1. **(valid-output — open list renders and resolve/dismiss removes item)** Given at least one `review_queue` item with `state = 'open'`, When the workbench renders, Then each open item displays its `kind` (one of `extraction`, `match`, `new_cluster`, `merge_candidate`) and its `reason` text, alongside the approve, correct, dismiss, and duplicate-card merge action controls; AND When the operator approves, corrects, or dismisses an item through the EPIC-04 review-queue resolve endpoint and the endpoint returns a success status (HTTP status code below 400), Then the item is removed from the open list and its `state` is set to `resolved` (or `dismissed` for a dismiss action).
2. **(input-validation — correction form)** Given the correction form for an item, When the operator submits it, Then the form validates that every required field is present before submit, and a submission with an empty required field is rejected with a labeled field error and no resolve request is sent to the endpoint.
3. **(error-handling — failed resolve)** Given a resolve action is submitted, When the resolve endpoint returns an error status (HTTP status code 500 or higher), Then a labeled error state renders and the item stays in the open list with its `state` unchanged.
4. **(edge-case — empty queue and concurrent/stale resolution)** Given zero `review_queue` items with `state = 'open'`, When the workbench renders, Then an `empty queue` state renders in place of the list grid; AND given an item that was already resolved by another action since it was loaded, When the operator submits a resolve for that item, Then a `stale item` message renders and the item is refreshed out of the open list rather than double-resolved.
5. **(security — escaped payload rendering)** Given a `review_queue.payload` that carries ingestion-derived external data, When the workbench renders the payload detail, Then each key and value is output as escaped text in key/value rows through the framework's default text interpolation and is never injected as HTML (no `dangerouslySetInnerHTML` or direct `innerHTML`); any URL-valued field is emitted as a link only when it parses as an absolute `https://` URL from a backend-vetted host allowlist, otherwise it renders as plain escaped text.
6. **(responsive — 375 CSS pixel viewport)** Given a viewport width of 375 CSS pixels, When the workbench renders, Then each open queue item stacks into a labeled single-column block, the correction form fields and the approve/correct/dismiss/merge controls stay within the 375-pixel width and reachable, the form does not overflow its container, and the page shows no page-level horizontal scrollbar.
7. **(accessibility — form labels, focus, and announcements)** Given the workbench renders, When assistive technology inspects it, Then every correction-form field has a programmatically associated `<label>`; the approve/correct/dismiss/merge controls are reachable by keyboard in DOM order with a visible focus indicator at a contrast ratio of at least 3:1; queue mutations and resolve errors are announced through an `aria-live="polite"`/`role="status"` region; and after an item is removed from the open list, keyboard focus moves to the next open item or to a defined queue-level landmark.
8. **(ui-state — action pending/disabled)** Given an approve/correct/dismiss or merge action, When the resolve or merge request is in flight, Then that item's action controls become disabled/pending to block a duplicate submission; When the request succeeds, Then the item is removed from the open list; When it fails, Then the controls return to enabled and a labeled error renders.

## Sub-tasks

- [ ] Build the operator-only workbench list view of open `review_queue` items (@frontend-engineer)
- [ ] Render each item's `kind`, `reason`, and `payload` detail as escaped key/value rows (no `dangerouslySetInnerHTML`/`innerHTML`), emitting URL-valued fields as links only when they parse as absolute `https://` URLs from a backend-vetted host allowlist (@frontend-engineer)
- [ ] Wire approve/correct/dismiss to the EPIC-04 review-queue resolve endpoint, disabling the item's controls while the request is in flight (@frontend-engineer)
- [ ] Wire the duplicate-card merge action (and the counterpart-override action) (@frontend-engineer)
- [ ] Stack each queue item into a labeled single-column block at a 375 CSS pixel viewport with no page-level horizontal scrollbar and no form overflow (@frontend-engineer)
- [ ] Associate a `<label>` with every correction-form field, give controls a visible keyboard focus indicator in DOM order, announce queue mutations/errors through an `aria-live="polite"`/`role="status"` region, and move focus to the next open item after removal (@frontend-engineer)
...
- [ ] The `review_queue.payload` renders as escaped key/value rows (no `dangerouslySetInnerHTML`/`innerHTML`); a URL-valued field is emitted as a link only when it parses as an absolute `https://` URL from a backend-vetted host allowlist, otherwise it renders as plain escaped text.
- [ ] At a 375 CSS pixel viewport each queue item stacks into a labeled single-column block, the correction form and action controls stay within width without overflow, and the page shows no page-level horizontal scrollbar.
- [ ] Every correction-form field has an associated `<label>`; action controls are keyboard-reachable in DOM order with a visible focus indicator at a contrast ratio of at least 3:1; queue mutations and errors are announced through an `aria-live="polite"`/`role="status"` region; and focus moves to the next open item after an item is removed.
- [ ] While a resolve or merge request is in flight, the item's action controls are disabled/pending to block a duplicate submission, returning to enabled with a labeled error on failure or removing the item on success.
...
- [ ] **Testing:** UI tests pass for the open-list render, resolve/dismiss/merge, the `empty queue` state, the resolve-error state, the concurrent/stale-item state, the escaped payload rendering, the 375 CSS pixel responsive layout, the accessibility semantics (form labels, focus management, aria-live announcements), and the in-flight disabled action state; the UI coverage target is **≥50%** where code applies.
