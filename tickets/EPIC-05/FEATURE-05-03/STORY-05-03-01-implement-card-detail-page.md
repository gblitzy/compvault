...
Environment access and the Vercel Preview (= dev/qa) deployment for this view are configured once in [FEATURE-05-01 — Frontend Foundation & Environment Access](../FEATURE-05-01-frontend-foundation-and-environment-access.md) per the canonical Blitzy environments reference <https://docs.blitzy.com/administration/environments>; this page additionally requires the EPIC-04 card-detail endpoint `GET /api/variations/{variation_id}` ([STORY-04-02-03](../../EPIC-04/FEATURE-04-02/STORY-04-02-03-implement-card-detail-and-sales-table.md)) to return both the recent-sales rows (`sales[]`) and the read-side `counterpart` block it renders — `counterpart_value`, `ratio`, `direction`, `source`, and `no_physical_counterpart`, with `source = override` taking precedence over `source = computed`. The counterpart-override endpoint ([STORY-04-03-02](../../EPIC-04/FEATURE-04-03/STORY-04-03-02-implement-counterpart-override-endpoint.md)) is the operator **write** side and is not read by this page. The full environment-configuration procedure is not duplicated here.
...
2. **(valid-output — counterpart panel and digital-only label)** Given a variation that has a digital↔physical counterpart, When the counterpart panel renders, Then it displays the counterpart's current value (`counterpart_value`) and the cross-format ratio (`ratio`, for example `physical is 3.2× the digital`) sourced from the card-detail endpoint's `counterpart` block, and renders the relationship in the stated `direction` (physical→digital and digital→physical); AND given a variation whose `parallel_type.format_availability = 'digital'` (for example a Gilded tier) with no physical counterpart (the `counterpart` block's `no_physical_counterpart` is `true`), Then a `digital-exclusive / no physical counterpart` label renders in place of a counterpart value.
...
- [ ] Render the digital↔physical counterpart panel from the card-detail endpoint's `counterpart` block (`counterpart_value`, `ratio`, `direction`, `source`, `no_physical_counterpart`), working in both directions (@frontend-engineer)
...
- **[STORY-04-03-02 — Implement Counterpart-Override Endpoint](../../EPIC-04/FEATURE-04-03/STORY-04-03-02-implement-counterpart-override-endpoint.md):** the EPIC-04 counterpart-override **write** endpoint; the override it records is surfaced to this page through the card-detail endpoint's `counterpart` block (`source = override` taking precedence over `source = computed`), not read directly here; epic context — **[EPIC-04 — Backend Application & API](../../EPIC-04-backend-application-and-api.md)**.
