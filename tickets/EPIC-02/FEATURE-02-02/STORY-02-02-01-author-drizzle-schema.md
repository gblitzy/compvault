...
- Model the `SMALLINT` identity exceptions (`grade.id`, `valuation.grade_id`, `valuation.window_days`, `sale_observation.grade_id`, `collection_item.grade_id`) and the nullable `owner_user_id` columns (`watchlist`, `collection_item`, `saved_search`) — `@database-engineer`
