...
> As a Platform Engineer, I want a migrate.yml workflow that rehearses migrations on the shared `dev-qa` Neon branch using the unpooled URL, so that schema changes are verified before they reach production and branch protection can require a green migration.

## Connection & Environment Note

The `migrate.yml` workflow applies migrations using the **unpooled** `DATABASE_URL_UNPOOLED` connection and never the pooled `DATABASE_URL` — mixing the pooled and unpooled connections breaks migrations, and that is the documented failure mode. The shared `dev-qa` Neon branch the rehearsal runs against and the encrypted `DATABASE_URL_UNPOOLED` secret the workflow consumes are provisioned per [FEATURE-02-01 — Neon Project & Branching Topology](../FEATURE-02-01-neon-project-and-branching-topology.md) (documented per <https://docs.blitzy.com/administration/environments>); the target Neon database runs **PostgreSQL 15+** because `valuation`'s `UNIQUE NULLS NOT DISTINCT (variation_id, grade_id, window_days, cost_basis)` is rejected on any server below version 15.
...
1. **(valid-output)** **Given** a pull request, **When** `migrate.yml` runs, **Then** it applies the migration set on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED` and reports success with exit 0.
2. **(valid-output)** **Given** a manual `workflow_dispatch` invocation with no open pull request, **When** `migrate.yml` runs, **Then** it applies the migration set on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED` with exit 0, and the workflow declares no cron trigger.
...
6. **(edge-case)** **Given** the workflow completes, **When** the run ends, **Then** the shared `dev-qa` branch is left intact (no per-run branch was created or torn down) and remains available for the next rehearsal.
...
- Wire the migration step to apply the generated migration set on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED` secret — `@devops-engineer`
...
- Confirm the rehearsal targets the shared `dev-qa` branch and that no per-run branch is created or torn down on either the success path or the failure path — `@devops-engineer`
...
- **Concurrent:** two pull requests rehearse at the same time → both run against the shared `dev-qa` branch and therefore share its state; per-run isolation is intentionally traded away (see the lost-isolation note).
...
- **[`STORY-02-01-02` — Configure Preview Deployments Against the Shared Dev/QA Branch](../FEATURE-02-01/STORY-02-01-02-configure-per-pr-preview-branches.md):** documents how Vercel Preview (dev/qa) deployments share the long-lived `dev-qa` Neon branch the rehearsal also targets.
- **[`STORY-02-01-03` — Configure CI Against the Shared Dev/QA Branch](../FEATURE-02-01/STORY-02-01-03-configure-per-ci-ephemeral-branches.md):** documents that CI runs against the shared long-lived `dev-qa` Neon branch this workflow also targets.
...
- `EPIC-06` `STORY-06-02-02` (API integration tests) runs this migration set on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED` (cited by identifier).

## Story Estimation Guidance

- **Effort: Medium** — one workflow file carrying a migration step, a fail-fast secret guard, and a schema-diff verification step, scoped to a fixed migration set and a fixed schema target (9 enums, 20 tables).
- **Complexity: Medium** — the unpooled-only connection discipline, the shared `dev-qa` connection used across both the success path and the failure path (no per-run branch lifecycle to manage), and the failed-check signalling each carry a defined pass/fail behavior the workflow must encode.
- **Uncertainty: Medium** — the CI runner plus connecting the rehearsal to the shared `dev-qa` branch is the open variable (no per-run branch is created or torn down); the migration set and the schema target are fixed by the upstream stories.
...
- [ ] **Testing:** A sample pull request triggers `migrate.yml`, which applies migrations on the shared `dev-qa` Neon branch via the unpooled `DATABASE_URL_UNPOOLED` (exit 0) and leaves the `dev-qa` branch intact, while a deliberately broken migration fails the check.
