...
This is the first and foundational story of FEATURE-02-01. It provisions the CompVault Neon Postgres project on **PostgreSQL 15+**, generates a **Neon API key** carrying branch create and delete permission, creates the **production (main) branch**, and stores the production **pooled `DATABASE_URL`** and **unpooled `DATABASE_URL_UNPOOLED`** as **encrypted secrets** in Blitzy. A Neon branch is a copy-on-write clone of its parent, so the production branch created here is the parent from which every per-PR preview branch ([STORY-02-01-02](STORY-02-01-02-configure-per-pr-preview-branches.md)), every per-CI ephemeral branch ([STORY-02-01-03](STORY-02-01-03-configure-per-ci-ephemeral-branches.md)), and the local development branch are later cloned. This is the **hard upstream prerequisite** for the rest of EPIC-02, for `EPIC-03` data writes, and for `EPIC-06`'s per-CI test branch; the pooled-versus-unpooled connection discipline this story establishes is documented in detail in [STORY-02-01-04](STORY-02-01-04-document-pooled-and-unpooled-connections.md).
...
- **Neon.** Create the Neon project on **PostgreSQL 15+**; generate a **Neon API key** carrying branch create and delete permission; create the **production (main) branch**; and retrieve the production branch's **pooled** connection string and its **unpooled / direct** connection string.
...
|----------|-----------------|---------------------------|
| Blitzy | Environment per target; plaintext variables and encrypted secrets | Store the pooled `DATABASE_URL`, the unpooled `DATABASE_URL_UNPOOLED`, and the Neon API key as encrypted secrets per <https://docs.blitzy.com/administration/environments> |
| Neon | Project plus an API key carrying branch create and delete permission; production (main) branch; pooled and unpooled connection strings | Provision the project on PostgreSQL 15+, create the production branch, and expose both the pooled and unpooled connection strings |
...
3. Create and confirm the **production (main) branch** as the single long-lived parent branch.
...
5. **(edge-case)** **Given** the production branch, **When** a preview or CI branch is later created, **Then** the production branch is its copy-on-write parent.
...
7. **(valid-output)** **Given** the production branch, **When** it is identified, **Then** it carries the deterministic name `main` and exactly 1 production branch exists as the single long-lived parent.
...
- Create and confirm the production (main) branch — `@platform-engineer`
...
- Sibling stories — every other FEATURE-02-01 story branches from the production branch created here:
  - **[STORY-02-01-02 — Configure Per-PR Preview Branches](STORY-02-01-02-configure-per-pr-preview-branches.md):** clones one copy-on-write preview branch from this production branch per pull request.
  - **[STORY-02-01-03 — Configure Per-CI Ephemeral Branches](STORY-02-01-03-configure-per-ci-ephemeral-branches.md):** clones a throwaway CI branch from this production branch at pipeline start and deletes it on completion.
...
- Cross-epic consumers (cited by identifier): all of `EPIC-02` schema, migration, and seed work runs against this project and production branch; `EPIC-03` data writes target these branches through the pooled `DATABASE_URL`; and `EPIC-06`'s per-CI test branch (`STORY-06-01-03`) clones from this production branch.
...
- [ ] The production (main) branch exists as the single long-lived parent branch.
