...
- **Rehearsal on the shared `dev-qa` branch.** The `migrate.yml` workflow applies the generated migration against the shared long-lived `dev-qa` Neon branch (provisioned in FEATURE-02-01) on the unpooled `DATABASE_URL_UNPOOLED` before merge. It is triggered by pull request and by manual dispatch; it carries no cron schedule.
...
3. **[STORY-02-02-03 — Create the Migration Rehearsal Workflow](FEATURE-02-02/STORY-02-02-03-create-migration-rehearsal-workflow.md)** — create a `migrate.yml` workflow that applies the migration on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED`, required green by EPIC-06 branch protection (`STORY-06-03-03`).
...
- **[FEATURE-02-01 — Neon Project & Branching Topology](FEATURE-02-01-neon-project-and-branching-topology.md):** provisions the Neon project on PostgreSQL **15+**, the protected `production` branch and the shared `dev-qa` branch the rehearsal runs against, and the unpooled connection string that DDL and migrations require.
...
- **EPIC-06 — Testing & CI/CD Quality Gates:** `STORY-06-03-03` branch protection on `main` requires `migrate.yml` to be green, and `STORY-06-02-02` integration tests run these migrations on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED`.
...
- [ ] `migrate.yml` applies the migration on the shared `dev-qa` Neon branch using the unpooled `DATABASE_URL_UNPOOLED` and exits 0; it is triggered by pull request and manual dispatch with no cron schedule.
- [ ] PostgreSQL **15+** is documented as required for `valuation`'s `UNIQUE NULLS NOT DISTINCT`.
- [ ] **Testing:** a migration rehearsal on the shared `dev-qa` Neon branch applies the generated migration through the unpooled `DATABASE_URL_UNPOOLED` with zero errors, and a schema diff against `docs/schema.sql` reports no missing enums and no missing tables.
