...
- [ ] **Testing:** an integration test on the shared `dev-qa` Neon branch opens the pooled client, runs a `SELECT 1` health query returning 1 row, and asserts that a missing `DATABASE_URL` throws a named error before any query is issued.
