...
8. **(input-validation / security — sanitized overwrite)** **Given** an on-conflict upsert that overwrites mutable scraped values (`title`, `asking_price`, `shipping_cost`, `currency`, `listing_url`, `primary_image_url`, `end_date`) from the incoming record, **When** the overwrite is applied, **Then** the update executes through a parameterized/ORM statement (zero string-concatenated SQL); `listing_url` and `primary_image_url` are overwritten only with an `http`/`https` URL on the eBay listing/image host allowlist (a non-allowlisted URL leaves the prior stored value intact and logs 1 rejection by `source_item_id` without the raw value); `asking_price` and `shipping_cost` are overwritten only with a non-negative value inside the `NUMERIC(12,2)` range and `currency` only with a 3-letter ISO-4217 or known-symbol value; and `title` is control-character-stripped and truncated to a 1000-character bound before the write.
...
- Validate and sanitize every incoming scraped value before the on-conflict overwrite — `http`/`https` URL allowlisting for `listing_url`/`primary_image_url`, non-negative `NUMERIC(12,2)` bounds for `asking_price`/`shipping_cost`, an ISO-4217/known-symbol check for `currency`, and control-character stripping with a 1000-character bound for `title`; on a rejected field leave the prior stored value intact and log the rejection by `source_item_id` without echoing the raw value. `@ingestion-engineer`
- Make the batch retry-safe so a partial-failure retry re-applies the same records without creating duplicate rows, routing the write through the pooled `DATABASE_URL` and never the unpooled `DATABASE_URL_UNPOOLED`, using parameterized/ORM statements only. `@ingestion-engineer`
...
- [ ] Incoming scraped values are validated and sanitized before the on-conflict overwrite — parameterized/ORM writes only, `http`/`https` URL allowlisting for `listing_url`/`primary_image_url`, non-negative `NUMERIC(12,2)` price bounds and ISO-4217/known-symbol `currency` validation, `title` control-character stripping and a 1000-character bound, and each rejected field left at its prior stored value and logged by `source_item_id` without the raw value; stored text is output-encoded by downstream UI/API consumers so a persisted value cannot execute as markup.
