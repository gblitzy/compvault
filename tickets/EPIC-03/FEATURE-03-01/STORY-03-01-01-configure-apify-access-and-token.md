...
- **[EPIC-01 — Environment & Configuration Foundation](../../EPIC-01-environment-and-configuration-foundation.md):** supplies the single Blitzy environment and the secrets baseline that stores `APIFY_TOKEN`. Specifically **FEATURE-01-01 (Blitzy Environment Provisioning)** manually configures and attaches the single environment, and **FEATURE-01-03 (Secrets & Variable Management)** establishes the encrypted-secret baseline; this story configures the Apify-specific access on top of that baseline.
- **No upstream data dependency.** This story is pure configuration and requires no database access: no database connection is read here, and no `raw_listing` write occurs.
