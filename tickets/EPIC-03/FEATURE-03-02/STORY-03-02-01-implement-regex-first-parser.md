...
This is the **first** of the four stories in FEATURE-03-02 (Extraction & Matching), implementing **Stage 1** of the two-stage extractor defined in PRD §7.6.2 (deterministic regex/rules first, LLM fallback second). The Stage-1 parser is the cheap, high-precision path: it reads one persisted `raw_listing` row — its `title`, its `item_aspects` JSONB (which carries the structured eBay aspects, including `Features: Digital`, Set, and Character), and its `description` — and extracts card identity into a single `extraction` row. It pulls serial (`15/50` → `ex_serial_number=15`, `ex_serial_run=50`), print run (`/25`, `1/1` → `ex_print_run`), year (→ `ex_year`), grade (PSA/CGC/BGS/SGC + value → `ex_grade`), and format (→ `ex_format`), and tokenizes candidate parallel and character text. It assigns `confidence >= 0.85` when structured aspects are present, so the majority of listings resolve here and never reach the LLM fallback (`STORY-03-02-02`), which runs only on the sub-threshold remainder (`confidence < 0.85`).
...
> **As a** Data/Ingestion Engineer, **I want** a deterministic regex/rules parser that extracts card identity from listing titles and eBay item aspects, **so that** the cheap, high-precision path resolves the majority of listings without invoking the LLM.
...
2. **(Valid-output / digital via aspect)** **Given** a `raw_listing` whose `item_aspects` contains `{"Features": "Digital"}`, **When** the parser runs, **Then** `ex_format='digital'` and `confidence >= 0.85` (the structured aspect is present).
...
6. **(Input-validation / empty)** **Given** a null or empty `title` and an empty `item_aspects`, **When** the parser runs, **Then** no identity tokens are extracted, `confidence` is set to a value below `0.85`, and one `extraction` row is still written (the parser does not throw).
...
- Implement format detection: read the `Features: Digital` aspect first (→ `ex_format`), else the keyword `SWCT`/`Card Trader`/`digital` flagged lower-confidence. `@data-eng`
...
- **Empty/Null:** a null or empty `title` together with an empty `item_aspects` → `confidence < 0.85`, one `extraction` row is still written, and the parser does not crash.
...
- **[STORY-03-01-02 — Invoke Actor & Persist Raw Listings](../FEATURE-03-01/STORY-03-01-02-invoke-actor-and-persist-raw-listings.md):** persists the `raw_listing` rows (`title`, `item_aspects`, `description`) this parser consumes; with no persisted raw listings there is nothing to extract.
...
- [ ] Format detection prefers the `item_aspects` `Features: Digital` aspect; the keyword-only path (`SWCT`/`Card Trader`/`digital`) is flagged with `confidence < 0.85`.
- [ ] `confidence >= 0.85` is assigned when structured aspects are present; below `0.85` otherwise.
...
- [ ] **Testing:** parser unit tests against title and item-aspect fixtures pass and meet a **≥90%** coverage target (exercised by EPIC-06 `STORY-06-02-01`), covering the `15/50` ambiguity, the `1/1` boundary, the empty/null title case, and a lot/bundle case.
