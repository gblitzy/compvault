...
- Build the request payload from the `raw_listing` `title`, `item_aspects`, and `description`. `@ml-eng`
...
- [ ] Requests carry the `raw_listing` `title`, `item_aspects`, and `description`; the response must satisfy the strict JSON schema (`character`, `set`, `year`, `card_number`, `parallel`, `print_run`, `serial`, `grade`, `format`) with a per-field confidence.
