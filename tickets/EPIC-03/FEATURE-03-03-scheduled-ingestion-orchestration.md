...
The daily run is hosted on **GitHub Actions, not a Vercel function** (Vercel serves request/response traffic only and times out on long ingests). The `ingest.yml` workflow declares `runs-on: ubuntu-latest`, `timeout-minutes: 120`, and a Node `>=18` runtime — the floor set by the `ebay-sold-listings` Apify actor (`engines.node` `>=18` in `apify/package.json`; container image `apify/actor-node:20`) — and is triggered by the cron schedule `0 8 * * *` (UTC; scheduled Actions run UTC-only and the spec records a 10–30 minute drift) plus the manual `workflow_dispatch` trigger. A keepalive step is included to prevent the 60-day scheduled-workflow auto-disable. The workflow reads its credentials from GitHub Actions encrypted secrets — `DATABASE_URL` (pooled, runtime), `APIFY_TOKEN` (actor invocation), and `LLM_API_KEY` (the batch LLM-fallback parser) — which are never committed to the repository.
...
3. Define the `ingest.yml` workflow on `runs-on: ubuntu-latest` with `timeout-minutes: 120`, a Node `>=18` runtime (the `ebay-sold-listings` Apify actor floor from `apify/package.json`; container image `apify/actor-node:20`), the cron `0 8 * * *` schedule, the `workflow_dispatch` trigger, and a keepalive step that prevents the 60-day auto-disable.
...
- [ ] The `ingest.yml` workflow runs on cron `0 8 * * *` (UTC) and on manual `workflow_dispatch`, declares `runs-on: ubuntu-latest`, `timeout-minutes: 120`, and a Node `>=18` runtime (the `ebay-sold-listings` Apify actor floor from `apify/package.json`), and is hosted on GitHub Actions, not a Vercel function.
