...
This feature creates the recurring orchestration that drives CompVault's ingestion pipeline: a single GitHub Actions workflow (`ingest.yml`) runs once per day on cron `0 8 * * *` (UTC) plus an on-demand `workflow_dispatch` trigger, invoking the Apify integration (FEATURE-03-01), then the extraction and matching logic (FEATURE-03-02), and finishing by recomputing the `valuation` price cache from `sale_observation`. The business value is a live, idempotent, resumable, budget-aware pipeline whose health is observable: every run writes one `ingestion_run` heartbeat row (`status` transitions `running` → `ok` or `failed`) and tracks `calls_used` against a hard daily request budget of ≤5,000 active source fetches, halting at 4,500 and resuming the next cycle so a run never exceeds the free-tier cap — on the active path `calls_used` counts the Apify actor's fetched eBay result pages/requests (bounded per search term by the actor input `maxPagesPerSearch` and globally by `maxItems`), while the deferred eBay Browse API contributes 0 to `calls_used` until `STORY-03-03-03` is unblocked — while a non-zero exit sets `ingestion_run.status = failed` and fails the Action so a missed or broken run is visible to the operator. Scope is limited to the schedule, the run heartbeat, the call-budget cap, and the documentation of the DEFERRED eBay API; this feature orchestrates — it does not implement — the Apify actor integration ([FEATURE-03-01 — Apify Actor Integration](FEATURE-03-01-apify-actor-integration.md)) or the two-stage extraction and confidence-gated matching ([FEATURE-03-02 — Extraction & Matching](FEATURE-03-02-extraction-and-matching.md)). This feature is delivered through **3 stories**.

## Environment Access & Configuration

The base platform access for ingestion — the single Blitzy environment and the Apify `APIFY_TOKEN` secret — is provisioned in [FEATURE-03-01 — Apify Actor Integration](FEATURE-03-01-apify-actor-integration.md) and documented per the canonical Blitzy environments reference: <https://docs.blitzy.com/administration/environments>. This feature does not duplicate those steps; it ADDS the orchestration platform — **GitHub Actions** — and names the **DEFERRED eBay Developer API**.

The daily run is hosted on **GitHub Actions, not a Vercel function** (Vercel serves request/response traffic only and times out on long ingests). The `ingest.yml` workflow declares `runs-on: ubuntu-latest`, `timeout-minutes: 120`, and a Node `>=20.20.2` runtime — the TypeScript application's toolchain floor (root `package.json` `engines.node`) that the runner's `jobs/ingest.ts` pipeline and valuation recompute execute under; the `ebay-sold-listings` Apify actor keeps its own `engines.node` `>=18` floor (in `apify/package.json`; container image `apify/actor-node:20`), which the runner's `>=20.20.2` satisfies — and is triggered by the cron schedule `0 8 * * *` (UTC; scheduled Actions run UTC-only and the spec records a 10–30 minute drift) plus the manual `workflow_dispatch` trigger. A keepalive step is included to prevent the 60-day scheduled-workflow auto-disable. The workflow reads its credentials from GitHub Actions encrypted secrets — `DATABASE_URL` (pooled, runtime), `APIFY_TOKEN` (actor invocation), and `LLM_API_KEY` (the batch LLM-fallback parser) — which are never committed to the repository.
...
The daily run stays within a hard budget of **≤5,000 source fetch requests per day and halts at 4,500**, tracked through `ingestion_run.calls_used`. On the active path that counter counts the Apify actor's fetched eBay result pages/requests, bounded per search term by the actor input `maxPagesPerSearch` and globally by `maxItems`; the deferred eBay Browse API contributes 0 to `calls_used` until `STORY-03-03-03` is unblocked, at which point Browse calls are counted in the same budget. This step-by-step configuration is completed in full **before** the scheduled job runs, and every data write the run performs depends on EPIC-02's Neon branching (`STORY-02-01-*`) — the two long-lived branches `production` and `dev-qa` — and schema being provisioned first, using the pooled `DATABASE_URL` rather than the unpooled `DATABASE_URL_UNPOOLED` reserved for DDL and migrations.
...
| Blitzy / Apify Platform | The single Blitzy environment provisioned in FEATURE-03-01 (`APIFY_TOKEN` encrypted secret; actor execution rights) | Reuse the base ingestion access per <https://docs.blitzy.com/administration/environments>; this feature invokes the FEATURE-03-01 integration on the schedule |
...
3. Define the `ingest.yml` workflow on `runs-on: ubuntu-latest` with `timeout-minutes: 120`, a Node `>=20.20.2` runtime (the TypeScript application toolchain floor from the root `package.json`, which also satisfies the `ebay-sold-listings` Apify actor's own `>=18` floor from `apify/package.json`; container image `apify/actor-node:20`), the cron `0 8 * * *` schedule, the `workflow_dispatch` trigger, and a keepalive step that prevents the 60-day auto-disable.
...
6. Validate the wiring with a single `workflow_dispatch` dry-run that writes one `ingestion_run` heartbeat row and exercises the call-budget cap on the relevant Neon branch (`dev-qa` for non-production runs, `production` for production), confirming the environment is provisioned before the first scheduled run.
...
2. **[STORY-03-03-02 — Implement Heartbeat & Call-Budget Cap](FEATURE-03-03/STORY-03-03-02-implement-heartbeat-and-call-budget-cap.md)** — write one `ingestion_run` heartbeat row per run (`status` `running` → `ok`/`failed`, with `items_seen`, `items_new`, `sales_recorded`, `queued_for_review`, `error_count`, and `calls_used`) and enforce the daily budget cap (≤5,000 active source fetches, halt at `calls_used` = 4,500, resume next cycle) — on the active path `calls_used` counts the Apify actor's fetched eBay result pages/requests bounded by `maxPagesPerSearch`/`maxItems`, while the deferred eBay Browse API contributes 0 until `STORY-03-03-03` is unblocked; a non-zero exit sets `ingestion_run.status = failed` and fails the Action.
...
- **EPIC-01 — Environment & Configuration Foundation:** supplies the single Blitzy environment and the GitHub Actions encrypted secrets (`DATABASE_URL`, `APIFY_TOKEN`, `LLM_API_KEY`) the `ingest.yml` workflow consumes.
...
- [ ] The `ingest.yml` workflow runs on cron `0 8 * * *` (UTC) and on manual `workflow_dispatch`, declares `runs-on: ubuntu-latest`, `timeout-minutes: 120`, and a Node `>=20.20.2` runtime (the TypeScript application toolchain floor from the root `package.json`, which also satisfies the `ebay-sold-listings` Apify actor's own `>=18` floor from `apify/package.json`), and is hosted on GitHub Actions, not a Vercel function.
...
- [ ] The run halts when `ingestion_run.calls_used` reaches 4,500 of the ≤5,000-per-day source-fetch budget and resumes the next cycle; on the active path `calls_used` counts the Apify actor's fetched eBay result pages/requests (bounded by `maxPagesPerSearch`/`maxItems`) and the deferred eBay Browse API contributes 0 until `STORY-03-03-03` is unblocked; the pipeline is idempotent and resumable.
...
- [ ] All data writes target the pooled `DATABASE_URL` on the relevant EPIC-02 Neon branch (`dev-qa` for non-production runs, `production` for production; `STORY-02-01-*`); no write uses the unpooled `DATABASE_URL_UNPOOLED` reserved for DDL and migrations.
...
- [ ] **Testing:** a `workflow_dispatch` dry-run exercises the heartbeat and call-budget-cap logic on the relevant Neon branch (`dev-qa`) and reports run telemetry — asserting one `ingestion_run` row is written, that `status` ends `ok` on a clean run and `failed` on a forced non-zero exit, and that the run halts at `ingestion_run.calls_used` = 4,500.
