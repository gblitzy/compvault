...
Validation runs first and short-circuits invalid input, so it issues no query of its own. On the success path the request reaches a handler that reads the **pooled `DATABASE_URL`** (never the unpooled `DATABASE_URL_UNPOOLED`, which EPIC-02 reserves for DDL and migrations), threads the operator `userId` from the `getUserId()` seam, and makes **no LLM call** — LLM-assisted extraction stays in the EPIC-03 batch jobs (PRD §7, §7.5). The concrete value domains come from `docs/schema.sql` and the PRD and are shared verbatim by every EPIC-04 route: the `format` toggle accepts `physical`, `digital`, or `both`, where `both` (or an omitted `format`) selects both columns — so although the `format_t` enum itself holds only `physical` and `digital`, the request-level toggle adds `both` to match the `format_availability` enum (`physical` | `digital` | `both`); the character-search term parameter is named `q` (a non-empty character term) and the character-search `mode` accepts only `autocomplete` or `resolve`; a price-history `window` accepts only `90` or `365` (`valuation.window_days`); a history `granularity` accepts only `day`, `week`, `month`, or `year` (FR-2 / FR-8); the review-queue `state` accepts only `open`, `resolved`, or `dismissed` (`review_state`), the review-queue `kind` accepts only `extraction`, `match`, `new_cluster`, or `merge_candidate` (`review_kind`), and the review-queue resolve `action` accepts only `approve`, `correct`, `dismiss`, `confirm`, or `merge`; numeric ids are positive `BIGINT` identity keys; and a `limit` is a bounded numeric page size in `[1, 100]`. Every rejection returns a **safe error envelope** — a stable, public response body whose `error` field names the offending parameter (or carries a public error code) and that exposes no stack trace, no raw SQL text, no database connection string, no secret, and no internal exception message; the full diagnostic detail is logged server-side in redacted form only.
...
4. **(edge-case — empty or missing required parameter)** **Given** a required parameter that is empty or absent (for example a character-search request with no `q` value), **When** the request is processed, **Then** the response is **HTTP 400** with an `error` field naming the missing parameter (`q`).
...
7. **(input-validation — value outside allowed domain)** **Given** a parameter whose value falls outside its allowed-value set (for example `format=foil`, `window=180`, or `granularity=hour`), **When** the request is processed, **Then** the response is **HTTP 400** with an `error` field naming the parameter, because `format` accepts only `physical`, `digital`, or `both`, `window` accepts only `90` or `365`, and `granularity` accepts only `day`, `week`, `month`, or `year`.
8. **(error-handling — safe error envelope)** **Given** any validation failure or any downstream handler error, **When** the error response is returned, **Then** the body is a stable public envelope whose `error` field names the offending parameter or carries a public error code, and the response contains no stack trace, no raw SQL text, no database connection string, no secret, and no internal exception message, while the underlying diagnostic detail is logged server-side in redacted form only.

## Sub-tasks

- [ ] Define a shared query-parameter validation schema/utility — per-parameter name, type, required flag, min/max bound, and allowed-value set (`format` ∈ {`physical`, `digital`, `both`} with `both`/omitted selecting both columns; `mode` ∈ {`autocomplete`, `resolve`}; `window` ∈ {`90`, `365`}; `granularity` ∈ {`day`, `week`, `month`, `year`}; `state` ∈ {`open`, `resolved`, `dismissed`}; `kind` ∈ {`extraction`, `match`, `new_cluster`, `merge_candidate`}; resolve `action` ∈ {`approve`, `correct`, `dismiss`, `confirm`, `merge`}; `limit` ∈ [`1`, `100`]) — reused by every EPIC-04 route (@api-engineer)
...
- [ ] Return every validation failure and downstream handler error through a shared **safe error envelope** — a stable public `error` field (the parameter name or a public error code) that carries no stack trace, no raw SQL text, no connection string, no secret, and no internal exception message — and log the redacted diagnostic detail server-side only; reuse this envelope across every EPIC-04 route (@api-engineer)
...
- **Empty/Null:** a required parameter that is empty or missing (for example `q=` or an absent `q`) → **HTTP 400** with an `error` field naming it, and no database query is executed.
...
- [ ] A value outside its allowed-value domain (`format` other than `physical`/`digital`/`both`; `window` other than `90`/`365`; `granularity` other than `day`/`week`/`month`/`year`; `mode` other than `autocomplete`/`resolve`; `state` other than `open`/`resolved`/`dismissed`; `kind` other than `extraction`/`match`/`new_cluster`/`merge_candidate`; resolve `action` other than `approve`/`correct`/`dismiss`/`confirm`/`merge`) returns **HTTP 400** with an `error` field naming the parameter.
- [ ] Every validation failure and downstream handler error is returned through the shared safe error envelope — a stable public `error` field (parameter name or public error code) with no stack trace, raw SQL, connection string, secret, or internal exception message — and the redacted diagnostic detail is logged server-side only; the search and detail endpoints in FEATURE-04-02 and the operator endpoints in FEATURE-04-03 reuse this envelope.
