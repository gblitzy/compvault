...
2. **(valid-output — row fields)** **Given** a `variation` that has at least one `sale_observation`, **When** the endpoint returns its row, **Then** the row carries the latest `sale_price` and the latest `sale_date` from the `LEFT JOIN LATERAL` (`ORDER BY sale_date DESC LIMIT 1`) subquery, the `grade` and `serial_number` when present, and the parallel badge (`parallel_type.name` + `print_run`, for example `Mojo /50`).
...
5. **(input-validation — format filter value)** **Given** a `format` filter value outside the allowed set (the `format_t` values `physical` and `digital`, plus the Both toggle option), **When** the request is processed, **Then** the response is HTTP **400** with an `error` field that names `format`, and no database query is run.
...
7. **(edge-case — zero sales)** **Given** a `variation` with **zero** `sale_observation` rows, **When** the endpoint returns its row, **Then** the latest-price field is `null` and the row is labeled as having no recorded sale (the `LEFT JOIN LATERAL` subquery yields no row), and the row is retained in its `variation.format` group rather than dropped.
...
- [ ] Validate the `format` filter value against the allowed set (`format_t` values `physical` / `digital` plus the Both toggle option), returning HTTP 400 with an `error` field that names `format` and running no query for any other value (@backend-engineer)
...
- **Invalid — `format` filter outside the allowed set:** a `format` filter value outside the `format_t` values (`physical` / `digital`) plus the Both toggle option → HTTP **400** with an `error` field that names `format`, and no query is run.
...
- [ ] Each row carries the latest `sale_price` and the latest `sale_date` from the `LEFT JOIN LATERAL` (`ORDER BY sale_date DESC LIMIT 1`) subquery, plus the `grade` and `serial_number` when present, the parallel badge (`parallel_type.name` + `print_run`), and `variation.rep_image_url`.
...
- [ ] A `variation` with zero `sale_observation` rows returns a `null` latest-price field labeled as no recorded sale, and the row is retained in its `variation.format` group.
