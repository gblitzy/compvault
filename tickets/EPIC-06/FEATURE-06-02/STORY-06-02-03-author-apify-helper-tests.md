...
These three helpers currently exist as non-exported local `const` functions inside the existing Apify actor source `apify/src/main.js`, which also runs top-level actor side effects on load: `await Actor.init()`, `await Actor.getInput()`, a `searchTerms` validation that throws on empty input, proxy creation, `await crawler.run()`, and `await Actor.exit()`. Because the helpers are unexported and the module is side-effectful, they cannot be imported or unit-tested directly as the file stands today. **Testability prerequisite:** before these unit tests are authored, the three pure helpers (and the supporting `firstText`) are exposed through a side-effect-free importable helper boundary — the recommended realization is to extract them into an importable module (for example `apify/src/helpers.js`) that `apify/src/main.js` then imports, which leaves the actor's scraping behavior unchanged; an acceptable alternative is exporting the helpers from `main.js` with actor execution guarded behind an entrypoint check. This boundary is coordinated with `STORY-06-01-01`. Authoring this ticket modifies no source files; the helper-boundary change belongs to executing this test story, not to writing this ticket. The helpers stay pure string/URL functions: `extractItemId` pulls the numeric eBay item id out of a listing URL, `parsePrice` turns a price caption into `{ priceRaw, priceMin, priceMax, currency }`, and `parseSoldDate` turns a sold-date caption into `{ soldDateRaw, soldDate }`. The string inputs are sourced both directly and from the `li.s-item` / `li.s-card` listing-card HTML fixtures established in `STORY-06-01-02`, and the suite runs under the Vitest harness from `STORY-06-01-01`.
...
8. **(coverage gate)** *Given* the helper suite runs under `vitest run` with coverage instrumentation over the side-effect-free helper boundary, *When* line coverage for `parsePrice`, `parseSoldDate`, and `extractItemId` is below the **≥90%** bar, *Then* the test command exits with a non-zero status and the build fails.

## Sub-tasks

- Expose the actor helpers through a side-effect-free importable helper boundary — extract `parsePrice`, `parseSoldDate`, `extractItemId`, and the supporting `firstText` into an importable module that `apify/src/main.js` consumes, leaving the actor's scraping behavior unchanged — coordinated with `STORY-06-01-01`, so the unit tests import the real helpers without running the scraper. `@platform-engineer`
...
- `apify/src/main.js` — the authoritative reference for the three helpers' behavior and return shapes; authoring this ticket does not modify it. Executing this test story exposes the helpers through the side-effect-free importable helper boundary described above (a minimal, behavior-preserving extraction/export coordinated with `STORY-06-01-01`), so the helper logic is carried over unchanged rather than rewritten or duplicated. `apify/package.json` fixes the actor as `type: module` on Node `>=18`.
...
- [ ] `apify/src/main.js` is the authoritative reference for the three helpers' behavior; the helper logic is exposed for testing through the side-effect-free importable helper boundary (a behavior-preserving extraction/export coordinated with `STORY-06-01-01`) rather than duplicated or rewritten, and the actor's scraping behavior is left unchanged.
- [ ] The helpers are imported through the side-effect-free helper boundary, so the suite triggers 0 actor side effects (no `Actor.init()`, `Actor.getInput()`, `searchTerms` validation, or `crawler.run()`) and opens 0 live proxy sessions.
