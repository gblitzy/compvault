...
4. **(error-handling — failure path)** *Given* a simulated database error on the per-CI Neon branch, *When* the endpoint handles the request, *Then* it returns an HTTP **5xx** status and the response body contains **0** stack traces, **0** connection strings, and **0** internal identifiers.
...
8. **(coverage gate)** *Given* the integration suite runs under `vitest run` with coverage instrumentation against the migrated per-CI Neon branch, *When* line coverage for the exercised API routes falls below the **≥75%** bar, *Then* the test command exits with a non-zero status and the build fails.

## Sub-tasks

- Seed deterministic catalog and sales fixtures (`character`, `card`, `card_character`, `parallel_type`, `variation`, `sale_observation`, `valuation`, `review_queue`) into the per-CI Neon branch before the suite runs. `@backend-test-engineer`
...
- **Concurrent:** 2 or more simultaneous requests against the same per-CI Neon branch return consistent results with 0 cross-request data bleed.

## Dependencies

- `STORY-06-01-03` (per-CI Neon branch wiring) — provisions the branch from the production branch, applies migrations through the unpooled `DATABASE_URL_UNPOOLED`, exposes the pooled `DATABASE_URL` to this suite, and tears the branch down on completion; it itself depends on `EPIC-02` `STORY-02-01-*` (the Neon branching topology hard prerequisite).
...
- **Effort: High** — the work spans deterministic seeding across 8 tables, multiple endpoint suites (search, two-column results, detail/sales, price-history, review queue), and the per-CI Neon branch lifecycle, which exceeds a single-endpoint test pass.
...
- [ ] Migrations are applied to a per-CI Neon branch before the suite runs, and the branch is torn down on completion (the lifecycle owned by `STORY-06-01-03`).
- [ ] The per-CI Neon branch reports a PostgreSQL server version of 15 or higher, so the `valuation` `UNIQUE NULLS NOT DISTINCT (variation_id, grade_id, window_days, cost_basis)` constraint is created with exit code 0.
...
- [ ] **Testing:** the integration suite runs green under `vitest run` against a per-CI Neon branch and reports line coverage ≥75%; line coverage below 75% exits non-zero and fails the build.
