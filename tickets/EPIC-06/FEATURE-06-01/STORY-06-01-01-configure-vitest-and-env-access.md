...
This is the first and foundational story of FEATURE-06-01. It stands up the single Vitest harness that every later EPIC-06 suite runs on, and it completes EPIC-06's mandatory environment access. The sibling stories — fixtures and boundary mocks ([STORY-06-01-02](STORY-06-01-02-establish-fixtures-and-boundary-mocks.md)) and the shared `dev-qa` Neon-branch wiring ([STORY-06-01-03](STORY-06-01-03-wire-integration-tests-to-neon-branch.md)) — build on the harness configured here.

## User Story

As a **Platform Engineer**, I want Vitest configured to run both the TypeScript application (Node `>=20.20.2`) and the JavaScript ES-module Apify actor (which keeps its own Node `>=18` floor) with environment access set up — the CI/Vitest runner pins Node `>=20.20.2`, satisfying both, so that every later EPIC-06 suite has one deterministic, documented place to run before any other EPIC-06 work proceeds.
...
|----------|-----------------|---------------------------|
| Blitzy | Single environment; plaintext variables and encrypted secrets | Store the test `DATABASE_URL` and CI credentials as encrypted secrets per <https://docs.blitzy.com/administration/environments> |
| Neon | Connection string of the shared `dev-qa` Neon branch | Supplies the value the harness reads as the test `DATABASE_URL`; the shared `dev-qa` branch wiring is owned by STORY-06-01-03 and depends on EPIC-02 `STORY-02-01-*` (referenced here, not duplicated) |
...
1. **(input-validation)** Given a Node runtime below 20.20.2 (for example Node 18.20.0), When harness setup runs, Then setup exits with a non-zero code and emits a message that names the `>=20.20.2` engine floor and the detected version.
...
- Document the Node `>=20.20.2` engine assertion (the TypeScript application floor; the Apify actor retains its own `>=18` floor from `apify/package.json`) that fails setup on a runtime below 20.20.2 — `@platform-engineer`
...
- **Boundary:** Node is exactly 20.20.2 → setup passes, because the `>=20.20.2` floor is inclusive of 20.20.2.
...
- **[STORY-06-01-03 — Wire Integration Tests to the Shared dev-qa Neon Branch](STORY-06-01-03-wire-integration-tests-to-neon-branch.md):** owns the shared `dev-qa` Neon branch wiring whose connection string becomes the test `DATABASE_URL` this harness reads; depends on EPIC-02 `STORY-02-01-*`.
...
- **Complexity: Medium** — the configuration must load the actor's side-effect-free ES-module helper boundary under the same runner as the TypeScript application with 0 CommonJS transform errors (without triggering the top-level `await Actor.init()`/`crawler.run()` entrypoint in `apify/src/main.js`), and must enforce both the Node `>=20.20.2` application floor and the `DATABASE_URL` guard.
- **Uncertainty: Low** — the runner (Vitest), the application runtime floor (Node `>=20.20.2`) is fixed by the root `package.json`, while the actor's own floor (Node `>=18`) and module type (`"type": "module"`) are fixed by `apify/package.json`, which leaves the resolution path defined ahead of implementation.
...
- [ ] A single Vitest configuration runs both the TypeScript application and the ES-module JavaScript Apify actor's helper boundary under Node `>=20.20.2` (the application floor; the Apify actor helper boundary retains its own `>=18` floor).
...
- [ ] The Node `>=20.20.2` floor is stated for the TypeScript application (root `package.json` `engines.node`) and the Apify actor's own `>=18` floor is stated (matching `apify/package.json`); the CI runner pins `>=20.20.2`, satisfying both.
...
- [ ] **Testing:** `vitest run` executes the TypeScript app suite and the JavaScript actor helper-boundary suite green under Node `>=20.20.2` (which satisfies the actor's own `>=18` floor).
