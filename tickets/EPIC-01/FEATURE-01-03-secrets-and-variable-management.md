...
This feature authors the **`.env.example`** template — none exists in the repository today, so this creation is in scope — enumerating every variable the application reads, and configures the **Vercel project environment variables** (preview and production scopes) and the **GitHub Actions encrypted secrets** (CI and the scheduled ingestion). The business value is one committed `.env.example` documenting every variable name, plus configured Vercel variables and GitHub Actions secrets, giving the runtime, preview, and CI environments one consistent, auditable configuration that every later epic reads instead of guessing variable names. This is the **second realization of EPIC-01's Environment Access & Configuration obligation** — Vercel plus GitHub Actions — complementing [FEATURE-01-01 — Blitzy Environment Provisioning](FEATURE-01-01-blitzy-environment-provisioning.md). Scope is limited to authoring `.env.example` and configuring the Vercel and GitHub Actions variables and secrets; it does **not** provision the Blitzy environments (that is [FEATURE-01-01 — Blitzy Environment Provisioning](FEATURE-01-01-blitzy-environment-provisioning.md)) and it does **not** scaffold the application (that is [FEATURE-01-02 — Application Scaffolding & Tooling](FEATURE-01-02-application-scaffolding-and-tooling.md)); it stores placeholder values only and never a real secret. This feature is delivered through **3 stories**.

## Environment Access & Configuration

All variable and secret configuration for this feature follows the canonical Blitzy environments reference: <https://docs.blitzy.com/administration/environments>. Per that reference, non-sensitive values are stored as **plaintext variables** and sensitive credentials are stored as **encrypted secrets**, and the environment is then attached to the project. **Blitzy holds the source-of-truth configuration**, which is mirrored out to **Vercel** (the application runtime and preview deployments) and to **GitHub Actions** (CI and the scheduled ingestion). Secrets are **never committed** to the repository — `.env`, `.env.local`, and `.env.*.local` are git-ignored, and `.env.example` carries placeholder values only.

### Platforms and access required

| Platform | Access required | Purpose in FEATURE-01-03 |
|----------|-----------------|--------------------------|
| Vercel | Project environment variables across the preview and production scopes | Expose the active variable set to every preview deployment (per PR) and to the production deployment (on `main`) so each deployed scope resolves the variables the application reads |
...
2. Configure the Vercel project environment variables for the preview and production scopes so each deployment resolves the active variable set (STORY-01-03-02).
3. Configure the GitHub Actions encrypted secrets the CI pipeline and the scheduled ingestion workflow read, mirrored from the Blitzy source-of-truth configuration per <https://docs.blitzy.com/administration/environments> (STORY-01-03-03).
...
2. **[STORY-01-03-02 — Configure Vercel Environment Variables](FEATURE-01-03/STORY-01-03-02-configure-vercel-env-vars.md)** — configure the Vercel project environment variables across the preview and production scopes so each deployment resolves the active variable set.
...
- **[FEATURE-01-01 — Blitzy Environment Provisioning](FEATURE-01-01-blitzy-environment-provisioning.md):** provisions the Blitzy Dev, Staging, and Prod environments that hold the source-of-truth secrets this feature mirrors out to Vercel and GitHub Actions.
...
- [ ] The Vercel project environment variables are configured across the preview and production scopes.
...
- [ ] Blitzy holds the source-of-truth configuration mirrored to Vercel and GitHub Actions, configured per <https://docs.blitzy.com/administration/environments>.
