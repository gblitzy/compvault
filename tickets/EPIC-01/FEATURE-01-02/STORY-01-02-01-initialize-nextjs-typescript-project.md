...
1. Confirm the `Dev`, `Staging`, and `Prod` environments from FEATURE-01-01 are attached and validated.
2. From the repository root, initialize a Next.js App Router project with TypeScript enabled.
3. Set `strict` to `true` in `tsconfig.json` and pin `engines.node` to `>=18` in `package.json`.
...
3. **(Input validation — Node floor)** **Given** the package manifest, **when** the `engines.node` field is read, **then** it specifies `>=18` and a Node runtime below major version 18 is rejected by the toolchain.
...
## Sub-Tasks
- [ ] Confirm the Blitzy environments from FEATURE-01-01 are attached and validated. `@platform-engineer`
- [ ] Initialize the Next.js App Router + TypeScript project at the repository root. `@platform-engineer`
- [ ] Set `strict: true` in `tsconfig.json` and pin `engines.node` to `>=18` in `package.json`. `@platform-engineer`
...
- **Boundary:** a Node major version of exactly 18 is accepted; major version 17 is rejected by the `>=18` floor.
...
- [ ] `engines.node` is `>=18` and `tsconfig.json` sets `strict: true`.
