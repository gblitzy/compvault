...
- **Platform:** GitHub Actions — encrypted repository secrets consumed by the CI workflow and the scheduled ingestion workflow via `${{ secrets.NAME }}`. Blitzy holds the source-of-truth configuration; this story mirrors the values to GitHub.
...
2. Create the four active encrypted secrets (`DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `APIFY_TOKEN`, `LLM_API_KEY`) using values mirrored from the Blitzy source-of-truth.
...
5. **(Valid output — migrations use unpooled)** **Given** the migration workflow, **when** it runs DDL, **then** it reads `DATABASE_URL_UNPOOLED` via `${{ secrets.DATABASE_URL_UNPOOLED }}`, while the runtime and ingestion steps read the pooled `DATABASE_URL`.
...
- [ ] Create the four active encrypted secrets (`DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `APIFY_TOKEN`, `LLM_API_KEY`) with values mirrored from the Blitzy source-of-truth. `@devops-engineer`
...
- [ ] Document which workflow reads the unpooled `DATABASE_URL_UNPOOLED` (migrations) versus the pooled `DATABASE_URL` (runtime and ingestion). `@devops-engineer`
...
- [ ] Migrations read `DATABASE_URL_UNPOOLED`; the runtime and ingestion steps read the pooled `DATABASE_URL`.
