...
- **Platform:** the `.env.example` file authored here is the committed template that documents the variable set later configured on **Vercel** (in [STORY-01-03-02](STORY-01-03-02-configure-vercel-env-vars.md)) and **GitHub Actions** (in [STORY-01-03-03](STORY-01-03-03-configure-github-actions-secrets.md)). Blitzy holds the source-of-truth configuration that those stories mirror to Vercel and GitHub.
...
### Variable set to enumerate (eight names, with status)
...
2. Create `.env.example` at the repository root listing the eight variable names with placeholder values.
...
1. **(Valid output)** **Given** the repository-root scaffold, **when** the Platform Engineer authors `.env.example` per <https://docs.blitzy.com/administration/environments>, **then** the file lists all eight variable names (`DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `APIFY_TOKEN`, `LLM_API_KEY`, `EBAY_CLIENT_ID`, `EBAY_CLIENT_SECRET`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`) with placeholder values and zero real secret values.
2. **(Valid output — status labels)** **Given** the eight variables, **when** each is documented, **then** the four active variables (`DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `APIFY_TOKEN`, `LLM_API_KEY`) are labeled `ACTIVE`, `EBAY_CLIENT_ID`/`EBAY_CLIENT_SECRET` are labeled `deferred`, and `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` are labeled `Phase 3`.
...
## Sub-Tasks
- [ ] Author `.env.example` at the repository root listing all eight variable names with placeholder values. `@platform-engineer`
...
- **Boundary:** `.env.example` must contain all eight variable names; seven names block completion.
...
## Definition of Done
- [ ] `.env.example` is committed at the repository root with all eight variable names and placeholder values.
...
- [ ] **Testing:** a secret-scan check and a documentation check confirm zero live credential values and all eight variable names are present before [STORY-01-03-02](STORY-01-03-02-configure-vercel-env-vars.md) and [STORY-01-03-03](STORY-01-03-03-configure-github-actions-secrets.md) mirror the set to Vercel and GitHub Actions.
